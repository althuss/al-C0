...
// I need to understand what is going on in these two locations and especially what this is used for, see position.h and .cc and utils/hashcat.h
 // Hm okay this just uses the HashCat function included from hashcat.h and has nothing to do with other stuff so should be okay as is.  Hash is just a long integer.
  uint64_t Hash() const {
  ...
  BitBoard queens() const { return queens_; }
  BitBoard our_knights() const {
    return our_pieces_ - pawns() - our_king_ - rooks_ - bishops_ - queens_;
  }
  BitBoard their_knights() const {
    return their_pieces_ - pawns() - their_king_ - rooks_ - bishops_ - queens_;
  }
